...
async def run_all_metrics(golden_dataset: dict, status_cb=None) -> dict:
    ...
    with logfire.span("Eval Phase 2 — All Metrics", total_samples=len(samples)):
       ...
       # Faithfulness
        if status_cb:
            status_cb(f"Faithfulness ({len(samples)} samples)...")
        ...
        #Answer Relevancy
        if status_cb:
            status_cb(f"Answer Relevancy ({len(samples)} samples)...")
        with logfire.span("Answer Relevancy"):
            inputs = [
        ...
        #Context Precision
        if status_cb:
            status_cb(f"Context Precision ({len(samples)} samples)...")
        with logfire.span("Context Precision"):
            inputs = [
                {
                    "user_input": s["question"],
        ...
        #Context Recall
        if status_cb:
            status_cb(f"Context Recall ({len(samples)} samples)...")
        with logfire.span("Context Recall"):
            inputs = [
                {
                    "user_input": s["question"],
                    "reference": s["reference"],
        ...
        #Answer Correctness
        if status_cb:
            status_cb(f" Answer Correctness ({len(samples)} samples)...")
        with logfire.span("Answer Correctness"):
            inputs = [
                {
                    "user_input": s["question"],
                    "response": s["actual_response"],
        ...
        # Tool Correctness
        if status_cb:
            status_cb("⚡ Tool Correctness (zero LLM calls)...")
